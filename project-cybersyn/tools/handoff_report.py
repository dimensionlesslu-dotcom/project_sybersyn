    ...
    for cat_name, cat_label in [("e_missing", "缺失"), ("e_wrong", "错误")]:
        for item in last_fb.get(cat_name, []):
            unmet.append({
                ...
                "deviation_types": deviation_types_from_feedback(last_fb),
            })
    return unmet
...
def render_markdown(state: dict, extra: dict) -> str:
    ...
    live_verdict = convergence_verdict(state)

    lines = [
        f"# 移交报告 — {state.get('task', '未命名')}",
        "",
        f"**生成时间**：{datetime.now(timezone.utc).isoformat()}  ",
        f"**实时状态**：{'[!] 已超 Nmax' if over_nmax else live_verdict.get('status', '?')} ({round_n}/{nmax})  ",
        f"**记录判定**：{state.get('verdict', '?')}  ",
    ...
    if unmet:
        ...
        for u in unmet:
            lines.append(f"| {u['category']} | {u['description'][:60]} | {u['severity']} | {u.get('deviation_types', [])} |")
        lines.append("")
    ...
    if history:
        ...
        for h in history:
            lines.append(f"| {h['round']} | {h['deviation_types']} | {h['e_missing']} | {h['e_wrong']} | {h['e_extra']} | {h['verdict']} |")
        lines.append("")
...
def render_json(state: dict, extra: dict) -> dict:
    ...
    return {
        ...
        "nmax": state.get("nmax"),
        "recorded_verdict": state.get("verdict"),
        "recorded_convergence": state.get("convergence", False),
        "live_verdict": live_verdict.get("status"),
        "delivery_allowed": live_verdict.get("delivery_allowed", False),

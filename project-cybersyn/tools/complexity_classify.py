...
def risk_vector(task: str, file_count: int, heterogeneity: bool,
                clarity: str, volatility: str) -> dict:
    """Emit qualitative risk signals; these are routing inputs, not probabilities."""
    task_lower = task.lower()
    irreversible = any(word in task_lower for word in ["不可逆", "迁移", "删除", "生产", "irreversible", "migration", "delete", "production"])
    high_impact = any(word in task_lower for word in ["安全", "认证", "支付", "关键", "security", "auth", "payment", "critical"])
    return {
        "scope": "high" if file_count > 5 else ("medium" if file_count > 2 else "low"),
        "heterogeneity": "high" if heterogeneity else "low",
        "requirement_ambiguity": {"clear": "low", "medium": "medium", "vague": "high"}[clarity],
        "verification_quality": "unknown",
        "environment_volatility": {"stable": "low", "uncertain": "medium", "volatile": "high"}[volatility],
        "reversibility": "low" if irreversible else "high",
        "failure_impact": "high" if high_impact else "medium",
    }
...
def classify(task: str, files: list, deps: list) -> dict:
    ...
    result = {
        "level": level,
        "factors": factors,
        "risk_vector": risk_vector(task, file_count, heterogeneity, clarity, volatility),
        "recommended": {
    ...
    if clarity == "vague":
        ...
        result["uncertainties"].append("需求验收标准含模糊表述")
    if volatility == "volatile":
        result["warnings"].append("环境波动性高，建议人工确认 L4 判定。")
        result["uncertainties"].append("外部环境可能在执行期间变化")
    if level == "L4":
    ...
    if len(set(tendencies)) >= 3:
        ...
        result["uncertainties"].append("文件、依赖、需求和环境信号指向不同复杂度")

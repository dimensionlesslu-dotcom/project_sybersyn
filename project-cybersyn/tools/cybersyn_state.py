...
from convergence_check import convergence_verdict
from evidence import canonicalize_feedback, deviation_types_from_feedback, validate_requirement_evidence
...
def _new_state_skeleton(task: str, level: str, nmax: int = None, audit_every: int = None,
                        enforce_from: dict = None, requirements: list = None) -> dict:
    """创建 v2.0 状态骨架。"""
    ...
    return {
        ...
        "_enforce": enforce,
        "r_t": {"requirements": list(requirements or []), "core_assumptions": [], "dead_zones": []},
...
STATE_MIGRATORS = {
    "1.0": lambda s: {
        **s,
        ...
        "requirement_evidence": s.get("requirement_evidence", []),
    }
}
...
def load_state(path: str = None) -> dict:
    ...
    while ver != STATE_SCHEMA_VERSION:
        ...
        ver = state["_version"]
    state.setdefault("requirement_evidence", [])
    for round_data in state.get("rounds", []):
        if isinstance(round_data, dict) and "feedback" in round_data:
            round_data["feedback"] = canonicalize_feedback(round_data["feedback"])
    return state
...
def _load_requirements(path: str) -> list[dict]:
    source = path[1:] if path.startswith("@") else path
    with open(source, "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    requirements = payload.get("requirements") if isinstance(payload, dict) else payload
    if not isinstance(requirements, list):
        raise ValueError("requirements 文件必须是数组，或包含 requirements 数组的对象")
    normalized = []
    seen = set()
    for index, item in enumerate(requirements):
        if isinstance(item, str):
            item = {"id": item, "description": item}
        if not isinstance(item, dict) or not item.get("id"):
            raise ValueError(f"requirements[{index}] 必须包含非空 id")
        requirement = dict(item)
        if requirement["id"] in seen:
            raise ValueError(f"requirements 重复 id: {requirement['id']}")
        seen.add(requirement["id"])
        normalized.append(requirement)
    return normalized


def _configured_requirement_ids(state: dict) -> list:
    return [
        item.get("id") if isinstance(item, dict) else item
        for item in state.get("r_t", {}).get("requirements", [])
    ] or None


def init_state(task: str, level: str, nmax: int = None, audit_every: int = None,
               enforce_from: str = None, path: str = None, requirements: list = None) -> dict:
    ...
    state = _new_state_skeleton(task, level, nmax, audit_every, enforce, requirements)
...
def update_round(state: dict, stage: str, data: dict, round_n: int = None) -> dict:
    ...
    data = deepcopy(data)
    if stage == "feedback":
        data = canonicalize_feedback(data)
    rounds = state["rounds"]
    if not rounds:
    ...
    if stage == "feedback":
        ...
        if "requirement_evidence" in data:
            issues = validate_requirement_evidence(
                data["requirement_evidence"], _configured_requirement_ids(state)
            )
            if issues:
                raise ValueError("requirement_evidence 无效: " + "; ".join(issues))
            state["requirement_evidence"] = data["requirement_evidence"]

    if stage == "output" and "requirement_evidence" in data:
        issues = validate_requirement_evidence(
            data["requirement_evidence"], _configured_requirement_ids(state)
        )
        if issues:
...
def compute_patterns_summary(state: dict) -> dict:
    ...
    for r in rounds:
        ...
        for dt in deviation_types_from_feedback(feedback):
            if dt in dist:
                dist[dt] += 1
...
def enforce_safety_boundaries(state: dict, action: str) -> None:
    ...
    if action == "modify" and len(rounds) >= 2:
        ...
        if deviation_types_from_feedback(f0) == ["A"] and deviation_types_from_feedback(f1) == ["A"]:
...
def summary_text(state: dict) -> str:
    ...
    lines = [
        ...
        f"记录判定: {state.get('verdict', '?')} | 收敛记录: {state.get('convergence', False)}",
        f"实时判定: {live.get('status', '?')} | 交付许可: {live.get('delivery_allowed', False)}",
...
def validate_state(state: dict) -> list[str]:
    issues = []
    required = ["_version", "task", "level", "round", "nmax", "r_t", "rounds", "audit_log", "requirement_evidence"]
    for k in required:
        if k not in state:
    ...
    issues.extend(validate_requirement_evidence(
        state.get("requirement_evidence"), requirement_ids
    ))
    return issues
...
def main():
    ...
    p.add_argument("--requirements", help="JSON requirements 文件；支持路径或 @路径")
    ...
    try:
        if args.command == "init":
            requirements = _load_requirements(args.requirements) if args.requirements else None
            state = init_state(args.task, args.level, args.nmax, args.audit_every,
                               args.enforce_from, args.output, requirements)

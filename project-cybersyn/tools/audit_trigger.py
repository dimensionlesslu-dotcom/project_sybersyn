...
from evidence import deviation_types_from_feedback
...
def check_same_type_persist(state: dict) -> tuple:
    ...
    recent_types = []
    for r in rounds[-3:]:
        types = tuple(sorted(deviation_types_from_feedback(r.get("feedback", {}))))
        if types:
            recent_types.append(types)
    if len(recent_types) >= 2 and recent_types[-1] == recent_types[-2]:
        return True, f"偏差类型 {list(recent_types[-1])} 连续 2 轮未消除"
    return False, ""
...
def check_type_mutation(state: dict) -> tuple:
    ...
    if len(rounds) < 2:
        return False, ""
    prev = set(deviation_types_from_feedback(rounds[-2].get("feedback", {})))
    curr = set(deviation_types_from_feedback(rounds[-1].get("feedback", {})))
    if prev.intersection({"A", "B"}) and curr.intersection({"C", "D"}):
        return True, f"偏差类型从 {sorted(prev)} 突变为 {sorted(curr)}"
    return False, ""


def check_stall(state: dict, convergence_result: dict = None) -> tuple:
    """过半 Nmax 未收敛。"""
    nmax = state.get("nmax", -1)
    if nmax == -1:
        return False, ""
    round_n = state.get("round", 0)
    live = convergence_result or convergence_verdict(state)
    if not live.get("delivery_allowed", False) and round_n > nmax / 2:
...
def generate_checklist(state: dict) -> list[str]:
    ...
    last_types = set(deviation_types_from_feedback(rounds[-1].get("feedback", {})))
    if "C" in last_types:
        base.insert(0, "[!] 当前为 C 型偏差（环境漂移）：r(t) 的哪些核心假设可能已被新输入推翻？")
    elif "D" in last_types:
        base.insert(0, "[!] 当前为 D 型偏差（涌现）：哪些子系统的交互假设需要重新检查？")
    elif last_types == {"A"}:
        type_a_count = sum(1 for r in rounds if deviation_types_from_feedback(r.get("feedback", {})) == ["A"])
        if type_a_count >= 3:
...
# ── 自动结论推荐 ──────────────────────────────────────────
def recommend_conclusion(state: dict, convergence_result: dict = None) -> dict:
    """基于偏差模式推荐审计结论。"""
    rounds = state.get("rounds", [])
    if not rounds:
        return {"recommended_conclusion": "maintain", "confidence": "low", "reasoning": "无迭代数据"}
    ...
    # C 型 + 环境信号 → reset-rt
    if "C" in last_types and env_signals > 0:
        return {
    ...
    type_a_count = sum(1 for r in rounds if deviation_types_from_feedback(r.get("feedback", {})) == ["A"])
    if live.get("status") == "stagnant" and check_same_type_persist(state)[0]:
        return {
            "recommended_conclusion": "restructure",
            "confidence": "high",
            "reasoning": "实时收敛结果为 stagnant 且同类偏差连续存在，不应推荐 maintain。",
        }
...
def main():
    ...
    try:
        ...
        live_convergence = convergence_verdict(state)
        ...
        triggers = [
            ...
            ("stall", lambda current: check_stall(current, live_convergence)),
        ...
        result = {
            ...
            "convergence": live_convergence,
        }

        if args.auto_conclude and triggered:
            rec = recommend_conclusion(state, live_convergence)
        ...
        if args.format == "text":
            ...
            if result.get("recommended_conclusion"):
                print(f"\n推荐结论: {result['recommended_conclusion']} (证据质量: {result['confidence']})")

...
def load_context(path: str) -> dict:
    ...
    return {
        ...
        "extra_count": sum(len(r.get("feedback", {}).get("e_extra", [])) for r in state.get("rounds", [])),
        "missing_count": sum(len(r.get("feedback", {}).get("e_missing", [])) for r in state.get("rounds", [])),
        "wrong_count": sum(len(r.get("feedback", {}).get("e_wrong", [])) for r in state.get("rounds", [])),
...
def render_json(task: str, strategies: list, divergence: dict, context: dict) -> dict:
    return {
        ...
        "evidence_policy": "策略模板和预测仅供假设审查，不得作为测试证据或收敛依据。",
    }

...
from evidence import (blocking_deviations, deviation_types_from_feedback,
                      evidence_summary, validate_requirement_evidence)
...
def extract_deviation_history(state: dict) -> list[dict]:
    ...
    for r in state.get("rounds", []):
        ...
        history.append({
            ...
            "deviation_types": deviation_types_from_feedback(fb),
            "requirement_evidence": fb.get("requirement_evidence", []),
...
def convergence_verdict(state: dict) -> dict:
    """判定收敛；RequirementEvidence 是自动收敛的必要条件。"""
    history = extract_deviation_history(state)
    current = history[-1] if history else {"e_missing": [], "e_extra": [], "e_wrong": [], "deviation_types": []}
    ...
    ledger = state.get("requirement_evidence") or current.get("requirement_evidence") or []
    configured_requirements = state.get("r_t", {}).get("requirements", [])
    requirement_ids = [
        item.get("id") if isinstance(item, dict) else item
        for item in configured_requirements
    ] or None
    ledger_issues = validate_requirement_evidence(ledger, requirement_ids)
    summary = evidence_summary(ledger)
    residuals = blocking_deviations(ledger)
    has_new = (comparison.get("new_items") or 0) > 0
    has_critical = any(d.get("severity") == "critical" for d in residuals)

    if ledger_issues:
        reasons.append("requirement_evidence 校验失败: " + "; ".join(ledger_issues))
    elif not ledger:
        reasons.append("缺少 requirement_evidence；不能自动判定为 converged")
    elif summary["unverified"]:
        reasons.append(f"存在 {summary['unverified']} 个 unverified requirement")
    elif residuals:
        reasons.append(f"存在 {len(residuals)} 个未消除的阻断性偏差")
    elif not has_new and all(r.get("status") == "passed" for r in ledger):
        status = "converged"
        reasons.append("所有 requirement 均 passed，且无 unverified/B/C/D 或高/中残留")

    if status != "converged" and ledger and not has_new and not has_critical and (residuals or summary["unverified"]):
        status = "stable_with_residuals"
    if previous and _total(current) > _total(previous):
        status = "diverging"
        reasons.append("当前偏差总量增加")
    elif previous and _total(current) == _total(previous) and _total(current) > 0 and not has_new:
        status = "stagnant"
        reasons.append("连续轮次偏差总量未减少")

    # 移交条件
    if status != "converged" and nmax != -1 and round_n >= nmax:
        status = "handoff"
        reasons.append(f"Nmax ({nmax}) 已达")

    if previous and current["deviation_types"] != previous.get("deviation_types", []):
        prev_types = set(previous.get("deviation_types", []))
        curr_types = set(current.get("deviation_types", []))
        if prev_types.intersection({"A", "B"}) and curr_types.intersection({"C", "D"}):
            status = "handoff"
            reasons.append(f"偏差类型从 {sorted(prev_types)} 突变为 {sorted(curr_types)}")

    if "D" in current["deviation_types"]:
    ...
    return {
        ...
        "delivery_allowed": status == "converged",
        "ledger_valid": not ledger_issues,
        "ledger_issues": ledger_issues,
        "requirement_evidence": summary,
    }
...
def _basic_recommendation(status: str, reasons: list) -> str:
    if status == "converged":
        return "已收敛，可进入交付验收门。"
    if status == "stable_with_residuals":
        return "状态稳定但仍有残留；显式说明后由用户决定，不能自动交付。"
    if status == "stagnant":
        return "连续轮次未改善，触发审计或更换策略。"
    if status == "diverging":
        return "偏差扩大，降低干预并触发审计或向人移交。"
    if status == "handoff":
...
def detect_env_drift(state: dict) -> dict:
    ...
    # C 型偏差连续 2+ 轮额外加分
    c_count = sum(1 for r in rounds if "C" in deviation_types_from_feedback(r.get("feedback", {})))
    if c_count >= 2:
        drift_score += 0.3

...
from evidence import evidence, requirement_record, now_iso
...
ALLOWED_TEST_RUNNERS = {
    "python", "python3", "py", "pytest", "pytest.exe", "node", "npm",
    "npx", "cargo", "go", "dotnet",
}
SHELL_TOKENS = {"|", "||", "&", "&&", ";", ">", ">>", "<", "2>", "2>>"}


def _parse_test_command(test_cmd: str) -> list[str]:
    """Parse a test command without giving it shell semantics."""
    if not isinstance(test_cmd, str) or not test_cmd.strip():
        raise ValueError("test_cmd 不能为空")
    if any(token in test_cmd for token in SHELL_TOKENS):
        raise ValueError("test_cmd 含有被禁止的 shell 控制符")
    argv = shlex.split(test_cmd, posix=False)
    if not argv:
        raise ValueError("test_cmd 解析为空")
    runner = os.path.basename(argv[0]).lower()
    if runner not in ALLOWED_TEST_RUNNERS:
        raise ValueError(f"不允许的测试执行器: {runner}")
    return argv


def run_test(test_cmd: str, timeout: int, *, execute: bool = False,
             authorized: bool = False, trusted_source: bool = False,
             cwd: str = None) -> tuple:
    """Return ``(passed, output)``; no command runs unless explicitly authorized."""
    try:
        argv = _parse_test_command(test_cmd)
    except ValueError as exc:
        return None, f"REJECTED: {exc}"
    if not execute:
        return None, "NOT_EXECUTED: test_cmd 仅作为待执行建议"
    if not authorized:
        return None, "NOT_AUTHORIZED: 缺少宿主 Agent 的执行授权"
    if not trusted_source:
        return None, "UNTRUSTED_SOURCE: test_cmd 来源未被标记为可信"
    try:
        result = subprocess.run(argv, shell=False, cwd=cwd, capture_output=True,
                                text=True, timeout=timeout)
        return result.returncode == 0, (result.stdout + "\n" + result.stderr).strip()
    except subprocess.TimeoutExpired:
        return None, "TIMEOUT"
    except OSError as exc:
        return None, f"EXECUTION_ERROR: {exc}"


def _test_evidence_summary(output: str, passed, cwd: str) -> str:
    exit_code = "0" if passed is True else ("nonzero" if passed is False else "unknown")
    workdir = cwd or os.getcwd()
    return f"cwd={workdir}; exit_code={exit_code}; output={output[:500]}"


def _execution_reason(output: str, test_cmd: str, timeout: int) -> str:
    if output == "TIMEOUT":
        return f"测试超时 ({timeout}s): {test_cmd}"
    labels = {
        "NOT_EXECUTED": "待宿主授权执行",
        "NOT_AUTHORIZED": "未取得执行授权",
        "UNTRUSTED_SOURCE": "命令来源不可信",
        "REJECTED": "命令不符合安全规则",
        "EXECUTION_ERROR": "执行环境错误",
    }
    prefix = output.split(":", 1)[0]
    return f"{labels.get(prefix, '测试未完成')}: {test_cmd}"
...
def compare_code(requirements: list, actual: str, baseline: str, timeout: int,
                 *, execute_tests: bool = False, authorize_test_execution: bool = False,
                 trusted_command_source: bool = False, cwd: str = None) -> dict:
    """代码模式主逻辑。"""
    ...
    for req in requirements:
        ...
        if test_cmd:
            passed, output = run_test(
                test_cmd, timeout, execute=execute_tests,
                authorized=authorize_test_execution,
                trusted_source=trusted_command_source, cwd=cwd,
            )
            if passed is None:
                unverified.append({
                    "requirement_id": req["id"],
                    "description": req.get("description", ""),
                    "reason": _execution_reason(output, test_cmd, timeout),
                    "severity": req.get("severity", "medium"),
                })
                requirement_evidence.append(requirement_record(
                    req["id"], "unverified", [evidence(
                        "inspection", "checklist_compare.run_test",
                        _test_evidence_summary(output, passed, cwd)
                    )], [{
                        "type": "B", "description": req.get("description", ""),
                        "severity": req.get("severity", "medium"),
                        "confidence": "high" if output.startswith("REJECTED") else "medium",
                    }]
                ))
            elif passed:
                covered += 1
                requirement_evidence.append(requirement_record(
                    req["id"], "passed", [evidence(
                        "test", test_cmd, _test_evidence_summary(output, passed, cwd)
                    )]
                ))
            else:
                wrong.append({
                ...
                requirement_evidence.append(requirement_record(
                    req["id"], "failed", [evidence(
                        "test", test_cmd, _test_evidence_summary(output, passed, cwd)
                    )], [{
                        "type": "A", "description": req.get("description", ""),
                        "severity": req.get("severity", "medium"),
                    }]
                ))
        else:
            ...
            if matched:
                ...
                summary = "关键词命中，仅形成 candidate；不能证明 requirement 已通过"
            else:
                unverified.append({
                    "requirement_id": req["id"],
                    "description": req.get("description", ""),
                ...
                summary = "无 test_cmd 且未检测到相关代码变更"
            requirement_evidence.append(requirement_record(
                req["id"], "unverified", [evidence(
                    "inspection", "checklist_compare.keyword_match", summary
                )], [{
                    "type": "B", "description": req.get("description", ""),
                    "severity": req.get("severity", "medium"),
                }]
...
def keyword_match(criterion: dict, actual: str) -> dict:
    ...
    status = "candidate" if rate > 0 else "unverified"
    ...
    for req in requirements:
        ...
        if result["status"] == "candidate":
            unverified.append({
                ...
                "reason": "关键词命中仅为 candidate，缺少直接验证证据",
                "severity": req.get("severity", "medium"),
            })
            requirement_evidence.append(requirement_record(
                req["id"], "unverified", [evidence(
                    "inspection", "checklist_compare.keyword_match",
                    f"candidate match_rate={result['match_rate']:.2f}"
                )], [{
                    "type": "B", "description": req.get("criterion", req.get("description", "")),
                    "severity": req.get("severity", "medium"),
                }]
            ))
        else:
            ...
            requirement_evidence.append(requirement_record(
                req["id"], "unverified", [evidence(
                    "inspection", "checklist_compare.keyword_match", "no direct evidence"
                )], [{
                    "type": "B", "description": req.get("criterion", req.get("description", "")),
                    "severity": req.get("severity", "medium"),
                }]
...
def _build_result(missing: list, extra: list, wrong: list, unverified: list,
                  total: int, covered: int, requirement_evidence=None) -> dict:
...
def main():
    ...
    parser.add_argument("--execute-tests", action="store_true",
                        help="显式执行 test_cmd；默认只记录待执行建议")
    parser.add_argument("--authorize-test-execution", action="store_true",
                        help="宿主 Agent 授权执行 test_cmd")
    parser.add_argument("--trusted-command-source", action="store_true",
                        help="确认 test_cmd 来自可信的需求源")
    parser.add_argument("--cwd", help="测试命令工作目录")
    ...
    try:
        ...
        if not requirements:
            ...
            result = compare_code(
                requirements, actual, args.baseline, args.test_timeout,
                execute_tests=args.execute_tests,
                authorize_test_execution=args.authorize_test_execution,
                trusted_command_source=args.trusted_command_source,
                cwd=args.cwd,
            )
        else:

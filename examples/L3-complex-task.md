...
```bash
python tools/complexity_classify.py --task "decouple auth/session/user" \
  --files auth.py,session.py,user.py --dependencies "auth,session,user" --format json
# → {"level": "L3", ...}

python tools/cybersyn_state.py init --level L3 --task "decouple auth/session/user"
```
...
```bash
python tools/cybersyn_state.py update --stage feedback \
  --data '{"e_missing":[{"description":"session expiry not routed via gateway","severity":"high"}],"e_extra":[],"e_wrong":[{"description":"2 integration tests fail","severity":"high"}],"deviation_type":"A"}'
```
...
```bash
python tools/cybersyn_state.py next-round
# ... 测试₁ / 反馈 / 修改 / 测试₂ 同上，每轮把 feedback 写入状态
python tools/convergence_check.py --state cybersyn_state.json --energy --format json
```
...
过三重验收门：① 验收清单逐条为空 ✅ ② 外部约束未变 ✅ ③ 核心假设最近经审计复核 ✅ → 交付。

```bash
python tools/cybersyn_state.py update --stage output --data '{"verdict":"converged","convergence":true}'
python tools/cybersyn_state.py archive
```
